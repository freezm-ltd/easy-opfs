...
var EventTarget2$1 = class EventTarget22 extends EventTarget {
  constructor() {
    super(...arguments);
    this.listeners = /* @__PURE__ */ new Map();
    this._bubbleMap = /* @__PURE__ */ new Map();
...
var Flowmeter = class extends EventTarget23 {
...
var AbstractSwitchableStream = class extends EventTarget23 {
...
var DuplexEndpoint = class _DuplexEndpoint extends EventTarget23 {
...
var ControlledReadableStream = class extends EventTarget23 {
...
class OpfsWorker {
  ...
  static async checkHandle(path) {
    const response = await this.instance.broadcastMessenger.request(path, null);
    return response.ok;
  }
  static async addHandle(request) {
    const workerResponse = await this.instance.workerMessenger.request("add", request);
    if (workerResponse.ok) return workerResponse;
    const broadcastResponse = await this.instance.broadcastMessenger.request(request.path, null);
...
class OpfsFile extends EventTarget2$2 {
  ...
  async read(at = 0, length) {
    const requestAsContext = { at, length };
    const readableGenerator = async (context) => {
      await this.init();
      ...
      const stream = response.data.pipeThrough(lengthCallback((delta) => {
        context.at += delta;
        if (context.length) context.length -= delta;
      }));
      return stream;
    };
    return retryableStream(readableGenerator, requestAsContext, { minSpeed: 0, minDuration: 5e3, slowDown: 0 });
  }
